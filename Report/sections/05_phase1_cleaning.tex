...
\subsection{Post-cleaning exploration (sanity check)}

After all three KNN passes the notebook re-explores the cleaned data
to make sure nothing weird happened. The post-clean pass prints:

\begin{itemize}
  \item a per-dataset null count (every numeric column is now CLEAN);
  \item the imputed-fields distribution \emph{before vs after} as a
        side-by-side histogram, so we can confirm the imputed values
        sit inside the existing distribution rather than tail-stretching
        it;
  \item a one-row sanity-check summary per dataset: mean daily
        boarding (around \stat{1.4k}), mean population per hex
        (around \stat{4.9k}), mean route length (around \stat{17 km}),
        mean fare (around \stat{4.7 LE}).
\end{itemize}

Nothing in this pass modifies a value. It is the second checkpoint
that keeps the cleaning honest: if the imputed boarding distribution
shifted noticeably to the right, or if a new dataset suddenly
contained NaN, this pass would catch it.
