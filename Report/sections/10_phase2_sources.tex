...
\subsection*{Exploration pass that runs after every source}

Each source block in \fn{Phase2\_Scraping\_Cleaning.ipynb} ends with a
short call to \fn{explore\_scraped\_data(label, path, key\_cols,
coord\_cols)}. This helper does the same five things every time, so
the audit is consistent across all eight sources:

\begin{itemize}
  \item prints row count and column types,
  \item flags any column whose null percentage is above zero,
  \item bounds the coordinate spread (lat/lon min, median, max) and
        compares it against the Cairo bbox,
  \item runs a numeric IQR pass to surface outlier candidates,
  \item shows the first three rows so we can eyeball the records.
\end{itemize}

The HTML for every audit chart is saved under
\fn{Phase2/Exports/scraping\_notebook\_visuals/} (file pattern
\fn{p2-s\textit{N}\_NN\_*.html}). Section 10 below quotes the most
important finding from each source's exploration. The full chart set
...
\textbf{Why we used it.} The LRT is the most discussed
