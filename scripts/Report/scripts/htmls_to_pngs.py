...
def make_driver():
    ...
    # NOTE: Do NOT pass --disable-gpu. Plotly's Scattergl/Heatmapgl traces
    # need a working WebGL context. With --disable-gpu they fall back to
    # the "WebGL not supported" placeholder. SwiftShader gives us
    # software WebGL inside headless Chrome.
...
def shoot(driver, html_path: Path, out_path: Path) -> bool:
    ...
    is_section_bundle = "notebook_sections" in html_path.parts and html_path.name.endswith("_notebook_visuals.html")
    time.sleep(FOLIUM_WAIT_S if folium else PLOTLY_WAIT_S)
    inject_dark_theme(driver)
    time.sleep(0.5)
    if is_section_bundle:
        # Resize window to the full document height and shoot the page.
        try:
...
# ---- Phase 2 Exports/notebook_sections (bundled per-question, 16 files) -> figures/phase2/sections/
# These are the canonical, up-to-date renders the report should use.
